...
const OPERATOR_SCHEDULES = {
  "op-anong": { startHour: 10, endHour: 21, slotMinutes: 30 },
  "op-bua": { startHour: 11, endHour: 20, slotMinutes: 30 },
  "op-dao": { startHour: 10, endHour: 22, slotMinutes: 30 }
};
function readOperators() {
  try {
    const raw = localStorage.getItem("operators");
    const def = [ { id: "op-anong", name: "Anong" }, { id: "op-bua", name: "Bua" }, { id: "op-dao", name: "Dao" } ];
    const list = raw ? JSON.parse(raw) : def;
    return Array.isArray(list) && list.length ? list.filter(o => o && o.id && o.name) : def;
  } catch {
    return [ { id: "op-anong", name: "Anong" }, { id: "op-bua", name: "Bua" }, { id: "op-dao", name: "Dao" } ];
  }
}
...
function populateOperatorSelect() {
  ...
  operatorSelect.innerHTML = OPERATORS.map(o => `<option value="${o.id}">${o.name}</option>`).join("");
}
...
if (operatorSelect) operatorSelect.addEventListener("change", () => populateAvailableTimes());
...
form.addEventListener("submit", (e) => {
  ...
  const operatorName = (() => { const ops = readOperators(); const op = ops.find(o => o.id === operatorId); return op ? op.name : null; })();
...
`;
  ...
  const payload = { date, start: time, minutes: duration === "0" ? 60 : Number(duration), name, phone, serviceId, notes, status: "pending", operatorId, operatorName };
...
function getAppointmentsForDate(iso, operatorId = null) {
  try {
    ...
    return list.filter(b => b.date === iso && b.status !== "cancelled" && (!operatorId || (b.operatorId ? b.operatorId === operatorId : true)));
  } catch {
    return [];
  }
}
...
function fetchAppointments(iso, operatorId = null) {
  ...
  const url = '/api/bookings?date=' + iso + (operatorId ? ('&operator=' + encodeURIComponent(operatorId)) : '');
  return fetch(url, { headers })
    .then(r => r.ok ? r.json() : Promise.reject())
    .then(list => list.filter(b => b.status !== 'cancelled' && (!operatorId || (b.operatorId ? b.operatorId === operatorId : true))).map(b => ({ start: b.start, end: b.end, status: b.status || 'pending', operatorId: b.operatorId || null })))
...
function saveBooking(b) {
  try {
    ...
    list.push({ id, date: b.date, start: b.start, end, name: b.name, phone: b.phone, serviceId: b.serviceId, minutes: b.minutes, notes: b.notes, status: b.status || "pending", operatorId: b.operatorId || null });
...
function renderSchedule() {
  ...
  fetchAppointments(iso, selectedOperator).then(appts => {
    for (let tMin = start; tMin < end; tMin += slot) {
      ...
      const overlapped = appts.find(a => toMin(a.start) < tMin + slot && toMin(a.end) > tMin);
      const opName = (() => {
